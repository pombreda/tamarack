-module(tamarack_app_endpoint_chart_handler).
...
resource_exists(Req, _State) ->
    ...
    {Endpoint, Req4} = cowboy_req:binding(endpoint, Req3),
    case tamarack_mapper:mapper(binary_to_atom(ChartType, utf8)) of
        undefined -> {false, Req4, undefined};
        {ok, MapperFn} -> case tamarack_model:application(AppKey) of
            undefined -> {false, Req4, undefined};
            #{id := AppId} -> {true, Req4, {MapperFn, AppId, Endpoint}}
        end
    end.

endpoint_data(Req, {MapperFn, AppId, Endpoint}) ->
    ...
    {ok, _, Rows} = epgpb:equery(main,
        "SELECT timestamp, key, SUM(value) FROM request_endpoint_by_minute
            WHERE app_id = $1 AND timestamp >= $2 AND timestamp <= $3 AND endpoint = $4
            GROUP BY app_id, timestamp, key
            ORDER BY timestamp, key",
        [AppId, FromTimestamp, ToTimestamp, Endpoint]),
